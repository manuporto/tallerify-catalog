...
const addAlbum = (playlistId, albumId) => {
  ...
  return generalHandler.createNewEntry(tables.playlists_albums, { album_id: albumId, playlist_id: playlistId });
};

const deleteAlbum = (playlistId, albumId) => {
  logger.debug(`Deleting album ${albumId} from playlist ${playlistId}`);
  return db(tables.playlists_albums).where({ playlist_id: playlistId, album_id: albumId }).del();
};
